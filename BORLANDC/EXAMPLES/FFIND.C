...
void            InitFfind(HANDLE hInstance, HANDLE hPrevInstance,
                          LPSTR lpszCmdLine, int cmdShow);
void            InitFfindFirst(HANDLE hInstance);
void            InitFfindAdded(HANDLE hPrevInstance);
void            InitFfindEvery(HANDLE hInstance, int cmdShow);
void            CloseFfind(void);

long FAR PASCAL FfindWndProc(HWND hWnd, WORD message,
...
void InitFfindEvery(HANDLE hInstance, int cmdShow)
{
    ...
    lpModelessProc = MakeProcInstance(MainDlgBoxProc, hInst);
                               ...
                               WORD wParam, LONG lParam)
{
    ...
    switch (message)
    {
        case WM_INITDIALOG:
            ...
            SendMessage(GetDlgItem(hDlg, QX_3), WM_SETFONT, hnewsfont, FALSE);
            SendMessage(GetDlgItem(hDlg, QX_LIST), WM_SETFONT, hnewsfont, FALSE);
            SendMessage(GetDlgItem(hDlg, QX_COUNT), WM_SETFONT, hnewsfont, FALSE);
            break;
